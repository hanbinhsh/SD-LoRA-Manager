...
| 状态 | 问题 ID | 本轮处理 |
|---|---|---|
| 已修复 | G-01 | 筛选时不再逐项删除/重插图库 item；仅更新变化的隐藏状态并一次失效 IconMode 几何缓存。筛选后回到首项，viewport 使用不透明主题背景。 |
| 已修复 | T-01、T-02、T-07 | Tag 浏览与模板 Tag Picker 统一为每张图片每个规范化 Tag 只计一次；大小写、空格、下划线、连字符等价；统一忽略 `__*` 缓存保留项。 |
| 已修复 | S-05 | 解密后严格检查明文头、总长度、JSON 长度和 JSON 文档类型；畸形包立即断开。 |
| 已修复 | S-09 | 仅在 `QFileSystemWatcher::addPath()` 成功后记录目录状态，并明确记录监控失败。 |
| 已修复 | S-10 | 根目录归属改为规范化绝对路径和目录边界匹配；重叠根目录优先最长匹配。 |
